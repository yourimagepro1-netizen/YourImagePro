...
import { UploadIcon, CheckCircleIcon, SparklesIcon, BriefcaseIcon, PaintBrushIcon, ZapIcon, UsersIcon, CameraIcon, HeartIcon } from './common/Icons';
...
type Category = 'couples_family_portraits' | 'lifestyle_editorial' | 'creative_portraits' | 'urban_hip' | 'seasonal' | 'sports_fitness' | 'memorial_remembrance';
...
const GroupUploadFlow: React.FC<GroupUploadFlowProps> = ({ user, onGenerate, onNavigateToPricing, generationCost, customGenerationCost, initialRequest }) => {
  ...
  const [showMemorialNotice, setShowMemorialNotice] = useState(false);
  ...
  const categories = useMemo(() => {
    return STYLE_OPTIONS.reduce((acc, style) => {
        if (style.category === 'couples_family_portraits' || style.category === 'lifestyle_editorial' || style.category === 'creative_portraits' || style.category === 'urban_hip' || style.category === 'seasonal' || style.category === 'sports_fitness' || style.category === 'memorial_remembrance') {
  ...
  const categoryDisplayNames: Record<Category, string> = {
    ...
    memorial_remembrance: 'Memorial & Remembrance'
  };
  ...
  const getCategoryIcon = (category: Category) => {
    ...
    switch (category) {
        ...
        case 'seasonal': return <SparklesIcon {...props} />;
        case 'sports_fitness': return <ZapIcon {...props} />;
        case 'memorial_remembrance': return <HeartIcon {...props} />;
        default: return null;
    }
  };
                    ...
                    <>
                        <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 border-b border-gray-700 mb-6">
                            {(Object.keys(categories) as Category[]).map(cat => {
                                ...
                                return (
                                <button key={cat} onClick={() => { setActiveCategory(cat); if(cat === 'memorial_remembrance') setShowMemorialNotice(true); }} className={`flex items-center capitalize px-4 py-2 text-sm font-semibold transition-colors rounded-t-lg border-b-2 ${activeCategory === cat ? 'text-fuchsia-400 border-fuchsia-400' : 'text-gray-400 border-transparent hover:text-white'}`}>
                        ...
                        {showMemorialNotice && (
                            <div className="bg-sky-900/30 border border-sky-700 text-sky-300 px-6 py-4 rounded-lg max-w-4xl mx-auto text-sm mb-6 text-center relative animate-fade-in">
                                <p className="font-bold">A Note on Memorial Images</p>
                                <p className="mt-2 pr-6">Creating these images can be an emotional process. Our AI will do its best to generate a respectful and beautiful tribute. Results work best with clear, front-facing photos of both individuals.</p>
                                <button onClick={() => setShowMemorialNotice(false)} className="absolute top-2 right-2 text-sky-300 hover:text-white font-bold text-2xl leading-none">&times;</button>
                            </div>
                        )}
