...
import { SparklesIcon, UploadIcon, UsersIcon } from './common/Icons';
...
const DesignStudioFlow: React.FC<DesignStudioFlowProps> = ({ user, onNavigateToPricing, onGenerate }) => {
  ...
  const [subjectPhotoPreviews, setSubjectPhotoPreviews] = useState<string[]>([]);
  ...
  const onSubjectsDrop = useCallback((acceptedFiles: File[]) => {
    setSubjectPhotos(currentPhotos => {
        const newPhotos = [...currentPhotos, ...acceptedFiles].slice(0, 3);
        ...
        })).then(newPreviews => setSubjectPhotoPreviews(newPreviews));
  ...
  const { getRootProps: getSubjectsRootProps, getInputProps: getSubjectsInputProps, isDragActive: isSubjectsDragActive } = useDropzone({
    onDrop: onSubjectsDrop,
    accept: { 'image/*': ['.jpeg', '.png', '.jpg', '.webp'] },
    maxFiles: 3,
    disabled: subjectPhotos.length >= 3,
  });

  const removeSubjectPhoto = (index: number) => {
      setSubjectPhotos(current => current.filter((_, i) => i !== index));
      setSubjectPhotoPreviews(current => current.filter((_, i) => i !== index));
  };
  ...
  const handleGenerate = () => {
    ...
    onGenerate({
      ...
      subjectPhotos: subjectPhotos.length > 0 ? subjectPhotos : undefined,
    });
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
        ...
        <div className="space-y-6">
          <div className="bg-gray-900/50 p-6 rounded-2xl border border-gray-800">
           ...
           <div className="bg-gray-900/50 p-6 rounded-2xl border border-gray-800">
             <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><UsersIcon className="w-6 h-6 text-fuchsia-400" /> Subject Photos (Optional)</h2>
             <p className="text-xs text-gray-400 mb-3">Add up to 3 photos of hosts, models, or main subjects.</p>
             <div className="flex gap-2 mb-2">
                {subjectPhotoPreviews.map((preview, index) => (
                    <div key={index} className="relative w-16 h-16 rounded-lg overflow-hidden">
                        ...
                        <button onClick={() => removeSubjectPhoto(index)} className="absolute top-0 right-0 bg-red-600/80 text-white rounded-full p-0 h-4 w-4 flex items-center justify-center font-bold text-xs leading-none">&times;</button>
                    </div>
                ))}
             </div>
             {subjectPhotos.length < 3 && (
                <div {...getSubjectsRootProps()} className={`p-4 border-2 border-dashed rounded-xl text-center cursor-pointer transition-colors ${isSubjectsDragActive ? 'border-fuchsia-500 bg-fuchsia-900/20' : 'border-gray-700 hover:border-fuchsia-400'}`}>
                    <input {...getSubjectsInputProps()} />
                    <UploadIcon className="h-8 w-8 text-gray-500 mx-auto" />
                    <p className="mt-1 text-sm text-gray-400">Drop photos here</p>
                </div>
             )}
           </div>
           <div className="bg-gray-900/50 p-6 rounded-2xl border border-gray-800">
             <h2 className="text-xl font-bold mb-4">Upload Logo (Optional)</h2>
             <div {...getLogoRootProps()} className={`p-4 border-2 border-dashed rounded-xl text-center cursor-pointer transition-colors ${isLogoDragActive ? 'border-fuchsia-500 bg-fuchsia-900/20' : 'border-gray-700 hover:border-fuchsia-400'}`}>
